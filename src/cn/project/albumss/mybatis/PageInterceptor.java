...
/**
 *
 * 分页拦截器，用于拦截�?��进行分页查询的操作，然后对其进行分页处理�?
 * 利用拦截器实现Mybatis分页的原理：
 * 要利用JDBC对数据库进行操作就必须要有一个对应的Statement对象，Mybatis在执行Sql语句前就会产生一个包含Sql语句的Statement对象，�?且对应的Sql语句
 * 是在Statement之前产生的，�?��我们就可以在它生成Statement之前对用来生成Statement的Sql语句下手。在Mybatis中Statement语句是�?过RoutingStatementHandler对象�?
 * prepare方法生成的�?�?��利用拦截器实现Mybatis分页的一个�?路就是拦截StatementHandler接口的prepare方法，然后在拦截器方法中把Sql语句改成对应的分页查询Sql语句，之后再调用
 * StatementHandler对象的prepare方法，即调用invocation.proceed()�?
 * 对于分页而言，在拦截器里面我们还�?��做的�?��操作就是统计满足当前条件的记录一共有多少，这是�?过获取到了原始的Sql语句后，把它改为对应的统计语句再利用Mybatis封装好的参数和设
...
public class PageInterceptor implements Interceptor {
	/**
	 * 数据库类型，不同的数据库有不同的分页方法，接收的内容：mysql、oracle
	 */
	...
	public Object intercept(Invocation invocation) throws Throwable {
       //对于StatementHandler其实只有两个实现类，�?��是RoutingStatementHandler，另�?��是抽象类BaseStatementHandler�?
       ...
       //PreparedStatementHandler或CallableStatementHandler，在RoutingStatementHandler里面�?��StatementHandler接口方法的实现都是调用的delegate对应的方法�?
       //我们在PageInterceptor类上已经用@Signature标记了该Interceptor只拦截StatementHandler接口的prepare方法，又因为Mybatis只有在建立RoutingStatementHandler的时�?
       //是�?过Interceptor的plugin方法进行包裹的，�?��我们这里拦截到的目标对象肯定是RoutingStatementHandler对象�?
    ...
    private String getOraclePageSql(Page page, StringBuffer sqlBuffer) {
       //计算第一条记录的位置，Oracle分页是�?过rownum进行的，而rownum是从1�?���?
    ...
    private String getCountSql(String sql) {
    	//返回结果集的sql可以看做�?���?
    	return "select count(*)  from (" + sql + ") tt_";
    }
